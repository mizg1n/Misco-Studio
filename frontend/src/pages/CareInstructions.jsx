    ...
    <div className="space-y-8 animate-fade-in pb-10">
      <div className="relative overflow-hidden glass-panel p-8 md:p-12 rounded-3xl border border-slate-700/50 shadow-2xl">
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-sm font-semibold mb-6">
            <HeartPulse size={16} />
            <span>Aftercare Rehberi</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-black text-white mb-6 tracking-tight">
            İyileşme Sürecinizi <br className="hidden md:block"/>
            <span className="bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">Mükemmelleştirin</span>
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {instructions.map((item, idx) => {
            ...
            return (
              <div 
                key={item.id} 
                className="group relative glass-panel p-8 rounded-3xl border border-slate-700/50 hover:border-purple-500/50 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-purple-500/10 overflow-hidden"
              >
                <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl -mr-10 -mt-10 transition-opacity duration-500 opacity-0 group-hover:opacity-100 ${isTattoo ? 'bg-indigo-500/20' : 'bg-emerald-500/20'}`}></div>
                
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg border ${
                      isTattoo 
                        ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400 shadow-indigo-500/20' 
                  ...
                  <h3 className="text-2xl font-bold text-slate-100 mb-4 group-hover:text-purple-300 transition-colors">
                    {item.title}
                  </h3>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-700/50 flex items-center gap-3">
                    <ShieldCheck className={isTattoo ? 'text-indigo-400' : 'text-emerald-400'} size={20} />
